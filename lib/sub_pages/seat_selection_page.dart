...
final List<SeatData> leftSideSeats = List.generate(
  20,
  (index) => SeatData("${index + 1}" "A", freeSeats.contains(index) ? true : false),
);
final List<SeatData> rightSideSeats = List.generate(
  20,
  (index) => SeatData("${index + 1}" "B", freeSeats.contains(index) ? true : false),
);
            ...
            return GestureDetector(
              onTap: () {
                if (!seats[i].isBooked) {
                  if (_selectedSeat.value != null) {
                    ...
                    Future.delayed(const Duration(milliseconds: 300), () => _selectedSeat.value = seats[i])
                        .whenComplete(
              ...
              child: Container(
                decoration: BoxDecoration(
                  color: _isSeatSelected(seats[i]) ? R.secondaryColor : Colors.transparent,
                  border: Border.all(
                    color: seats[i].isBooked ? R.tertiaryColor : R.secondaryColor,
                  ),
